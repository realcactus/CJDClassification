...
import numpy as np
from model.modules import get_token_embeddings, mlp, score_layer, text_cnn
...
class TextCNN(object):
    ...
    def __init__(self, config):
        ...
        self.embeddings_c = get_token_embeddings(self.config.vocab_size_c,
                                                 self.config.embed_matrix,
                                                 self.config.embedding_dim)
        # self.embeddings = get_token_embeddings(self.config.vocab_size_w,
        #                                        self.config.embed_matrix,
        #                                        self.config.embedding_dim)
        self.enc = tf.nn.embedding_lookup(self.embeddings_c, self.input_x_c)  # (N, T1, d_model)
        ...
        # CNN
        self.max_pool = text_cnn(inputs=self.enc,
        ...
        self.fnn_pool = tf.layers.dense(self.fnn_pool, self.config.hidden_dim, activation=tf.nn.relu)
